...
<body>
    <div class="cont-form-buy">
            <form class="form-buy">
                ...
                <div>
                <label class="title-buy"for="totalPrice">Subtotal: <?php echo "$". $subtotal ?></label>
                <div>
                <?php if($discount!=0){
                    ?>
                ...
                <label class="title-buy" for="totalPrice">Total Price:</label>
                    <label class="form-label-buy" type="text" id="totalPrice" name="totalPrice" class="" placeholder="<?php /*echo "$ $totalPrice";*/?>"> </label>
                </div>
                </form>
                <form class="form-buy" action="<?php echo FRONT_ROOT ?>Purchase/ProcessBuy" method="POST">
                <label type="hidden" name="quantity" value="<?php echo $quantity; ?>">
                <label type="hidden" name="idProjection" value="<?php echo $projection->getIdProjection(); ?>">
                <label type="hidden" name="subtotal" value="<?php echo $subtotal; ?>">
                <label type="hidden" name="discount" value="<?php echo $discount; ?>">
                <label type="hidden" name="totalPrice" value="<?php echo $totalPrice; ?>">
